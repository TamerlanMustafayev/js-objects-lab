...
const starterPokemonsArray = [] // Exercise is asking me to pick one starter Pokemon, so I made this array to store them all then pick one from

pokemon.forEach(starterPokemon => {
  if (starterPokemon.starter === true) {
    starterPokemonsArray.push(starterPokemon) 
    // console.log(starterPokemon)
  } 
})

game.party.push(starterPokemonsArray[0]) // 0a because I am only picking one pokemon that is starter
...
game.party.push(pokemon[1], pokemon[3], pokemon[44]) // I did not understand the instructions clearly here
...
game.gyms.forEach((city) => {
  if (city.difficulty < 3) {
    city.completed = true
  }
})
...
*/
game.party.forEach(eachPokemon=> {
...
for (everyPokemon of pokemon) {
  if (everyPokemon.starter) {
